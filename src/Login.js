...
import { Zap, BarChart2, CreditCard, Target } from 'lucide-react';

export default function Login({ onLogin }) {
  const features = [
    { icon: Zap,        color: '#7c3aed', label: 'Clasificación automática'        },
    { icon: BarChart2,  color: '#0891b2', label: 'Insights de tus patrones reales' },
    { icon: CreditCard, color: '#b45309', label: 'TC y créditos sincronizados'     },
    { icon: Target,     color: '#10b981', label: 'Cero fricción'                   },
  ];
      ...
      <div style={{
        ...
        {features.map((f, i) => (
          <div key={i} style={{
            ...
            background:'#f9f8ff', borderRadius:12, padding:'11px 14px',
            ...
            <span style={{fontSize:13,color:'#1f1b4b',fontWeight:500}}>{f.label}</span>
          </div>
        ))}
