...
export default function App({ user, data, onLogout }) {
  ...
  const [cuotasPagando, setCuotasPagando] = useState({});
  ...
  const handleTxSubmit = () => {
    ...
    if (tipoEspecial === 'disposicion') {
      ...
      if(addTx) addTx(ingTx).catch(err=>console.error('[App] addTx disposición falló:', err));
      if(addTx) addTx(intTx).catch(err=>console.error('[App] addTx interés disposición falló:', err));
    ...
    // No reclasificar en modo edición: la categoría ya guardada manda salvo
    // que el usuario la cambie explícitamente (ver elegirCategoria / handleTxEdit).
    if (!txEditId && txForm.tipo === 'gasto' && autoClasif && !autoClasif.elegida) {
    ...
    if (txForm.tipo === 'gasto' && medioPago.startsWith('tc')) {
      ...
      if(updateTc && tcs) {
        ...
        if(tc) updateTc(tc.id, { ...tc, consumido: tc.consumido+monto, deudaActual: tc.deudaActual+monto }).catch(err=>console.error('[App] updateTc gasto con TC falló:', err));
      }
    }
  ...
  const handleTxEdit = (t) => {
    if(String(t.id).startsWith('temp_')) return; // aún no confirma Firestore, espera un segundo
    setTxForm({...t,monto:String(t.monto)}); setTxEditId(t.id);
    // elegida:true evita que la UI muestre "no reconocí la categoría" y protege
    // la categoría original si el usuario retoca la descripción sin querer cambiarla.
    setAutoClasif(t.tipo==='gasto' ? {categoria:t.categoria, elegida:true} : null);
    setAlertaEmoc(null); setAlertaAceptada(false);
  ...
  const handlePagarCuota = (p) => {
    if(cuotasPagando[p.id]) return; // ya hay un pago de esta cuota en curso, evita doble registro
    if(!window.confirm(`¿Registrar pago de ${fmt(p.cuotaMensual)} para ${p.nombre} ${p.numero}?`)) return;
    setCuotasPagando(prev=>({...prev, [p.id]:true}));
    const fecha = hoyPeru();
    ...
    const limpiarFlag = () => setCuotasPagando(prev=>{ const n={...prev}; delete n[p.id]; return n; });
    if(updatePrestamo) {
      updatePrestamo(p.id, updP)
        .catch(err=>console.error('[App] updatePrestamo (pago cuota) falló:', err))
        .finally(limpiarFlag);
    } else {
      limpiarFlag();
    }
              ...
              {txForm.tipo==='gasto' && !tipoEspecial && (
                <div style={{marginBottom:14}}>
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
                    {[{id:'efectivo',label:'Efectivo / Cuenta',sub:'Resta tu balance'}, ...(tcs.length > 0 ? tcs.map(tc=>({id:`tc_${tc.id}`,label:`${tc.banco} ${tc.numero}`,sub:'Sube deuda TC'})) : [{id:'tc',label:'Tarjeta de crédito',sub:'Agrega tu TC en Deudas'}])].map(mp=>(
              ...
              {tipoEspecial === 'disposicion' && (
                <div style={{background:'#fef2f2',border:'1.5px solid #fecaca',borderRadius:14,padding:'12px 14px',marginBottom:12}}>
                  ...
                  <div style={{fontSize:12,color:'#991b1b',lineHeight:1.5}}>
                    Se registrará como ingreso y subirá tu deuda en {tcs[0]?`tu ${tcs[0].banco} ${tcs[0].numero}`:'tu tarjeta de crédito'}.<br/>
                    {tcs[0] ? <><strong>TEA: {tcs[0].tea}%</strong> — intereses desde el primer día.</> : 'Agrega tu tarjeta en Deudas para ver la TEA exacta.'}
                  </div>
                </div>
              )}
              {tipoEspecial === 'pagoTC' && (
                <div style={{background:'#eff6ff',border:'1.5px solid #bfdbfe',borderRadius:14,padding:'12px 14px',marginBottom:12}}>
                  ...
                  <div style={{fontSize:12,color:'#1e40af',lineHeight:1.5}}>
                    El monto se descontará del consumido de {tcs[0]?`tu ${tcs[0].banco} ${tcs[0].numero}`:'tu tarjeta de crédito'}.
                  </div>
                </div>
              )}
          ...
          <div>
            ...
            <div style={{padding:'16px'}}>
              ...
              {subTabDeudas==='prestamos' && (
                <div>
                  {prestamos.map(p=><PrestamoCard key={p.id} prestamo={p} onEdit={()=>handlePres2Edit(p)} onDelete={()=>handlePres2Del(p.id)} onPagar={()=>handlePagarCuota(p)} pagando={!!cuotasPagando[p.id]}/>)}
                ...
                return (
                  <div style={{background:'linear-gradient(135deg,#0f766e,#0d9488)',borderRadius:18,padding:'16px',marginBottom:14,boxShadow:'0 4px 20px rgba(13,148,136,0.2)'}}>
                    ...
                    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginTop:4}}>
                      <button onClick={()=>{sugs.forEach(s=>{setPresupuestos(prev=>[...prev,{id:'temp_'+Date.now()+Math.random(),categoria:s.categoria,limite:s.sugerido}]);if(addPres) addPres({categoria:s.categoria,limite:s.sugerido}).catch(err=>console.error('[App] addPres (sugerencia) falló:', err));});setSugerenciasDescartadas(true);}}
          ...
          <div>
            ...
            <div style={{padding:'16px'}}>
              ...
              {(()=>{
                ...
                if (topCat && topCat[0] !== 'Deporte') insights.push({ color:'#7c3aed', bg:'#f5f3ff', border:'#ddd6fe', icon:'Search', titulo:`${topCat[0]} es tu mayor gasto`, texto:`Representa el ${Math.round(topCat[1]/stats.gastos*100)}% de tus gastos totales — equivale a ${fmt(topCat[1])} este mes.` });
                ...
                if (cuotasMes > 0 && stats.ingresos > 0) {
                  ...
                  const prestamosActivos = prestamos.filter(p=>p.capitalPendiente>0);
                  const proximoATerminar = prestamosActivos.length>0
                    ? [...prestamosActivos].sort((a,b)=>(a.totalCuotas-a.cuotaActual)-(b.totalCuotas-b.cuotaActual))[0]
                    : null;
                  const cuotasRestantesProximo = proximoATerminar ? proximoATerminar.totalCuotas - proximoATerminar.cuotaActual : null;
                  const textoCuotas = `Pagas ${fmt(cuotasMes)}/mes en créditos — el ${ratio}% de tus ingresos.`
                    + (proximoATerminar && cuotasRestantesProximo <= 6
                        ? ` Al terminar el crédito ${proximoATerminar.numero} (${cuotasRestantesProximo} cuotas más) recuperas ${fmt(proximoATerminar.cuotaMensual)}/mes.`
                        : '');
                  insights.push({ color: ratio>30?'#f97316':'#6366f1', bg: ratio>30?'#fff7ed':'#f5f3ff', border: ratio>30?'#fed7aa':'#ddd6fe', icon:'Clock', titulo:`Tus cuotas = ${diasTrabajo} días de trabajo`, texto:textoCuotas });
                }
              ...
              {(() => {
                ...
                const fechas = [
                  ...
                  { label:'Pago TC', fecha:ciclo.limitePago, dias:diasPago, color:urgP, Icon:CreditCard, sub:tcs[0]?`Fecha límite de pago ${tcs[0].banco}`:'Fecha límite de pago de tu TC' },
              ...
              {(()=>{
                const casiTermina = prestamos.find(p=>p.cuotaActual>=p.totalCuotas-5&&p.cuotaActual>0);
                if(!casiTermina) return null;
                const cuotasRestantes = casiTermina.totalCuotas - casiTermina.cuotaActual;
                return (
                  <div style={{background:'#f0fdf4',border:'1.5px solid #bbf7d0',borderRadius:20,padding:'16px',marginBottom:14}}>
                    ...
                    <div style={{fontSize:13,color:'#166534',lineHeight:1.6}}>
                      El crédito {casiTermina.numero} tiene solo {cuotasRestantes} {cuotasRestantes===1?'cuota restante':'cuotas restantes'}. Al terminar tendrás <strong>{fmt(casiTermina.cuotaMensual)} extra libre por mes</strong>.
                    </div>
                  </div>
                );
              })()}
